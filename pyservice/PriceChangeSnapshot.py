...
def PriceChangeLive():
	...
	dfreport = concat([df['Quote'], df["QuoteType"], df["QuoteId"], df["QuoteGroup"], calcLIVE(df), calcOfficial(df, tday)],axis=1)
...
def reporthtml(df, ts):
	...
	"""
	bottom_idx = [7, 11, 18, 21, 24, 26, 26, 28]
...
if __name__ == '__main__':
	...
	while t_now < datetime.strptime(dstr+"7", "%Y%m%d%H"):
		...
		cc=['fx_hybrid_support_asia@asia.bnpparibas.com']
